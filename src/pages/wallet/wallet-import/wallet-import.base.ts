...
// import { OckPrivateKey, OckPublicKey } from 'ock-ts';
// import { MlcPrivateKey, MlcPublicKey } from 'mlc-ts';
...
/* export abstract class OckBaseWalletImport {
  ...
  protected import(address?: string, passphrase?: string, checkBIP39Passphrase?: boolean): void {
    ...
    if (address) {
      ...
      publicKey = OckPublicKey.fromAddress(address);
      publicKey.setNetwork(this.networkProvider.currentNetwork);
    } else {
      // test: import from passphrase here
      if (checkBIP39Passphrase && !this.validateMnemonic(passphrase)) {
      ...
      privateKey = OckPrivateKey.fromSeed(passphrase, this.networkProvider.currentNetwork);
      publicKey = privateKey.getPublicKey();
      address = publicKey.getAddress();
    }
...
/* export abstract class MlcBaseWalletImport {
  ...
  protected import(address?: string, passphrase?: string, checkBIP39Passphrase?: boolean): void {
    ...
    if (address) {
      ...
      publicKey = MlcPublicKey.fromAddress(address);
      publicKey.setNetwork(this.networkProvider.currentNetwork);
    } else {
      // test: import from passphrase here
      if (checkBIP39Passphrase && !this.validateMnemonic(passphrase)) {
      ...
      privateKey = MlcPrivateKey.fromSeed(passphrase, this.networkProvider.currentNetwork);
      publicKey = privateKey.getPublicKey();
      address = publicKey.getAddress();
    }
