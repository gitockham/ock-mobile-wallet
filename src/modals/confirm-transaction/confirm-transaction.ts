...
/* export class OckConfirmTransactionModal implements OnDestroy {
  ...
  broadcast() {
    ...
    this.ngZone.run(() => {
      this.hasBroadcast = true;
      this.ockApiProvider.postTransaction(this.transaction).subscribe(() => {
        this.dismiss(true);
      }, (error) => {
          this.translateService.get(
            ['TRANSACTIONS_PAGE.ERROR.NOTHING_SENT', 'TRANSACTIONS_PAGE.ERROR.FEE_TOO_LOW'],
            { fee: OckUtility.subToUnit(this.transaction.fee) }
          ).subscribe(translations => {
            let message = error.message;
...
/* export class MlcConfirmTransactionModal implements OnDestroy {
  ...
  broadcast() {
    ...
    this.ngZone.run(() => {
      this.hasBroadcast = true;
      this.mlcApiProvider.postTransaction(this.transaction).subscribe(() => {
        this.dismiss(true);
      }, (error) => {
          this.translateService.get(
            ['TRANSACTIONS_PAGE.ERROR.NOTHING_SENT', 'TRANSACTIONS_PAGE.ERROR.FEE_TOO_LOW'],
