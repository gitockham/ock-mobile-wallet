...
/* export class OckWalletListPage implements OnDestroy {
  @ViewChild('walletSlider') slider: Slides;
  @ViewChild(Content) content: Content;
  @ViewChild('chart') chart: BaseChartDirective;
  ...
  private loadWallets() {
    ...
    for (const w of lodash.values(this.currentProfile.wallets)) {
      const wallet = new Wallet().deserialize(w);
      if (OckPublicKey.validateAddress(wallet.address, this.currentNetwork)) {
        list.push(wallet);
      }
    }
...
/* export class MlcWalletListPage implements OnDestroy {
  @ViewChild('walletSlider') slider: Slides;
  @ViewChild(Content) content: Content;
  @ViewChild('chart') chart: BaseChartDirective;
  ...
  private loadWallets() {
    ...
    for (const w of lodash.values(this.currentProfile.wallets)) {
      const wallet = new Wallet().deserialize(w);
      if (MlcPublicKey.validateAddress(wallet.address, this.currentNetwork)) {
        list.push(wallet);
      }
    }
