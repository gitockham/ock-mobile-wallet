...
export class OckUtility {
  public static getRandomInt(min: number, max: number): number {
    return Math.floor(Math.random() * (max - min + 1)) + min;
  }

  public static ocktoshiToOck(ocktoshi: number, returnRaw: boolean = false): number {
    let result: number = ocktoshi / constants.WALLET_UNIT_TO_SATOSHI;

    if (!returnRaw) {
      result = Number((new BigNumber(result.toString())).toFixed(constants.OCKTOSHI_DP));
    }
