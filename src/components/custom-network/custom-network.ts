...
/* export class OckCustomNetworkComponent implements OnInit {
  ...
  public networks: { [networkId: string]: OckNetwork };
  public networksIds: string[];
  public networkChoices: { name: string, id?: string }[] = [];
  ...
  public createNewModal(): void {
    ...
    modal.onDidDismiss((result: OckNetwork) => {
      if (!result) {
        return;
      }
      this.openManageDialog(result);
  ...
  private openManageDialog(network: OckNetwork, networkId?: string) {
    const modal = this.modalCtrl.create('CustomNetworkEditModal', {network: network, id: networkId});
    modal.onDidDismiss((result: EditNetworkResult) => {
      if (!result) {
        return;
...
/* export class MlcCustomNetworkComponent implements OnInit {
  ...
  public networks: { [networkId: string]: MlcNetwork };
  public networksIds: string[];
  public networkChoices: { name: string, id?: string }[] = [];
  ...
  public createNewModal(): void {
    ...
    modal.onDidDismiss((result: MlcNetwork) => {
      if (!result) {
        return;
      }
      this.openManageDialog(result);
  ...
  private openManageDialog(network: MlcNetwork, networkId?: string) {
    const modal = this.modalCtrl.create('CustomNetworkEditModal', {network: network, id: networkId});
    modal.onDidDismiss((result: EditNetworkResult) => {
      if (!result) {
        return;
