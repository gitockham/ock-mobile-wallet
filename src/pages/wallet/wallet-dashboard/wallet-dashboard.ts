...
// import { OckNetwork, OckFees, OckTransactionDelegate, OckPrivateKey, OckTransactionType } from 'ock-ts';
// import { MlcNetwork, MlcFees, MlcTransactionDelegate, MlcPrivateKey, MlcTransactionType } from 'mlc-ts';
...
/* export class OckWalletDashboardPage implements OnInit, OnDestroy {
  ...
  private createDelegate(keys: WalletKeys) {
    const publicKey = this.wallet.publicKey || OckPrivateKey.fromSeed(keys.key).getPublicKey().toHex();

    const transaction = <OckTransactionDelegate>{
      passphrase: keys.key,
      secondPassphrase: keys.secondKey,
      username: this.newDelegateName,
      publicKey
...
/* export class MlcWalletDashboardPage implements OnInit, OnDestroy {
  ...
  private createDelegate(keys: WalletKeys) {
    const publicKey = this.wallet.publicKey || MlcPrivateKey.fromSeed(keys.key).getPublicKey().toHex();

    const transaction = <MlcTransactionDelegate>{
      passphrase: keys.key,
      secondPassphrase: keys.secondKey,
      username: this.newDelegateName,
      publicKey
