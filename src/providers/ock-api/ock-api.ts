...
export class OckApiProvider {
  ...
  public onUpdateDelegates$: Subject<ockts.Delegate[]> = new Subject<ockts.Delegate[]>();
  ...
  public get delegates(): Observable<ockts.Delegate[]> {
  ...
  public fetchDelegates(numberDelegatesToGet: number, getAllDelegates = false): Observable<ockts.Delegate[]> {
    if (!this._api) { return; }
    const limit = 51;
      ...
      if (totalAmount > balance) {
        ...
        observer.error({
          ...
          parameters: {
            ...
            fee: OckUtility.acktoshiToAck(transaction.fee),
