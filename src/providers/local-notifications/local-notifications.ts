...
/* export class OckLocalNotificationsProvider {
  ...
  watchTransactions(wallets: any) {
    for (const address in wallets) {
      const wallet = wallets[address];
      // Convert object to class
      const network = new ockts.Network();
      Object.assign(network, wallet.network);

      // Request list of transactions in the specific network
      const client = new ockts.Client(network);
      client.transaction.list({ senderId: address, recipientId: address })
        .subscribe(response => {
          if (!response.success) {
            return;
          ...
          if (Number(response.count) > wallet.transactions.length) {
            ...
            w.loadTransactions(response.transactions, this.ockApiProvider.network);
...
/* export class MlcLocalNotificationsProvider {
  ...
  watchTransactions(wallets: any) {
    for (const address in wallets) {
      const wallet = wallets[address];
      // Convert object to class
      const network = new mlcts.Network();
      Object.assign(network, wallet.network);

      // Request list of transactions in the specific network
      const client = new mlcts.Client(network);
      client.transaction.list({ senderId: address, recipientId: address })
        .subscribe(response => {
          if (!response.success) {
            return;
          ...
          if (Number(response.count) > wallet.transactions.length) {
            ...
            w.loadTransactions(response.transactions, this.mlcApiProvider.network);
