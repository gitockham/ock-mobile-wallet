...
/* export class OckAccountAutoCompleteService implements AutoCompleteService {
  ...
  private isValidContact(contact: AutoCompleteAccount, keyword: string): boolean {
    return OckPublicKey.validateAddress(contact.address, this.userDataProvider.currentNetwork)
           && (contact.address.toLowerCase().indexOf(keyword) > -1
               || (contact.name && contact.name.toLowerCase().indexOf(keyword) > -1));
  }
} */

/* export class MlcAccountAutoCompleteService implements AutoCompleteService {
  ...
  private isValidContact(contact: AutoCompleteAccount, keyword: string): boolean {
    return MlcPublicKey.validateAddress(contact.address, this.userDataProvider.currentNetwork)
           && (contact.address.toLowerCase().indexOf(keyword) > -1
               || (contact.name && contact.name.toLowerCase().indexOf(keyword) > -1));
  }
} */
