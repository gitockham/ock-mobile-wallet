...
// import { OckNetworkType } from 'ock-ts/model';
// import { OckPublicKey } from 'ock-ts/core';
// import { MlcNetworkType } from 'mlc-ts/model';
...
/* export class OckProfileSigninPage implements OnDestroy {
  @ViewChild('pinCode') pinCode: PinCodeComponent;
  ...
  load() {
    ...
    this.addresses = lodash(this.profiles).mapValues((o) => [o.name, o.networkId]).transform((result, data, id) => {
      ...
      const networkName = lodash.capitalize(network.name);
      const isMainnet = network.type === OckNetworkType.Mainnet;
  ...
  private profileHasWallets(profileId: string): boolean {
    ...
    const network = this.networks[profile.networkId];
    for (const wallet of lodash.values(profile.wallets)) {
      if (OckPublicKey.validateAddress(wallet.address, network)) {
        return true;
      }
    }
...
/* export class MlcProfileSigninPage implements OnDestroy {
  @ViewChild('pinCode') pinCode: PinCodeComponent;
  ...
  load() {
    ...
    this.addresses = lodash(this.profiles).mapValues((o) => [o.name, o.networkId]).transform((result, data, id) => {
      ...
      const networkName = lodash.capitalize(network.name);
      const isMainnet = network.type === MlcNetworkType.Mainnet;
  ...
  private profileHasWallets(profileId: string): boolean {
    ...
    const network = this.networks[profile.networkId];
    for (const wallet of lodash.values(profile.wallets)) {
      if (MlcPublicKey.validateAddress(wallet.address, network)) {
        return true;
      }
    }
